...
int main(int argc, char* argv[])
{
	...
	cv::Mat imgL = cv::imread("E:\\code\\OpenGL\\Reconstuction3d\\Reconstuction3d\\view1s.jpg"); 
	cv::Mat	imgR = cv::imread("view5s.jpg");
	imshow("l",imgL);
	//waitKey(0);
	...
	if (imgL.cols > stdWidth * 1.2)
	{
		...
		cv::Mat imgL1,imgR1;
		resize(imgL, imgL1, cv::Size(), resizeScale, resizeScale);
		resize(imgR, imgR1, cv::Size(), resizeScale, resizeScale);
		imgL = imgL1.clone();
		imgR = imgR1.clone();
	}
...
void SavePtsPairs( vector<cv::Point2f> &ptsL, vector<cv::Point2f> &ptsR, string &filename ) 
{
	...
	vector<cv::Point2f>::iterator iterL = ptsL.begin(),
		iterR = ptsR.begin();
	os<<ptsL.size()<<endl;
